# Continia Docs — Validation Rules

Canonical, actionable pass/fail rules derived from `style-guide.md` (the authoritative base
guide) and `style-guide-supplement.md` (docs-site mechanics and templates). This is the single
spec consumed by both the `docs-validator` lint script (`scripts/lint_article.py`) and Claude's
judgment pass. When either source file changes, update this file so they stay in sync.
...
| FM04 | `id` matches `<PREFIX>-` followed by digits, where `<PREFIX>` is the product's id prefix (e.g. `CB-130`, `DC-178`). | BLOCKING | [script] |
...
| HD02 | The H1 text matches the frontmatter `title` (ignoring a trailing " in [Solution name]" / minor wording). | INFO | [script] |
...
| ID | Rule | Severity | Enforcement |
|----|------|----------|-------------|
| VT01 | No invalid contractions (`there'd`, `they'd`, `you'd`, `it'd`, `ain't`). Valid-list contractions (`you'll`, `won't`, `it's`, `can't`, `don't`, ...) are allowed per style guide §9.3. | WARNING | [script] |
...
| VT06 | No em dashes (`—`); use an en dash (`–`) or restructure the sentence (style guide §9.2). | WARNING | [script] |
| VT07 | No Latin abbreviations (`e.g.`, `i.e.`, `etc.`, `et al.`); restructure the sentence instead (style guide §9.2). | WARNING | [script] |
| VT08 | No exclamation marks in body text (style guide §9.7). | WARNING | [script] |
| VT09 | No `(s)` plural suffix; use the plural or "one or more" (style guide §9.6). | WARNING | [script] |
...
| UI03 | Product names (e.g. "Continia Banking", "Business Central") are plain text, never bold/italic. | INFO | [judgment] |
...
| CO02 | Hint `style` is one of `info`, `success`, `danger`, `warning`. | BLOCKING | [script] |
...
| CO05 | Callout style matches intent (info=skim-worthy context, danger=crucial/"Important", warning=risk needing immediate attention, success=optional tip). | INFO | [judgment] |

## F. Links & cross-references

| ID | Rule | Severity | Enforcement |
|----|------|----------|-------------|
| LK01 | Internal article links use `[text](@<PREFIX>-###)` form; `###` is digits. Flag `@`-targets with no number or a malformed prefix. | BLOCKING | [script] |
...
| LK06 | Links under Related information are bare `[text](@<PREFIX>-###)` lines (no bullets). | INFO | [judgment] |

## G. Images & media

| ID | Rule | Severity | Enforcement |
|----|------|----------|-------------|
| IM01 | Image paths point under the product's image folder `/images/<PREFIX>/` (derived from the article's own `id` prefix). | WARNING | [script] |
...
| ID | Rule | Severity | Enforcement |
|----|------|----------|-------------|
| TB01 | Changelog `ID` column values are 5-digit work-item numbers, not `<PREFIX>-###` article ids. | WARNING | [judgment] |
...
- VT01 (invalid contractions) and CO03 (legacy alerts) stay WARNING: the corpus complies and we
  want them caught in new content. (VT01 originally banned all contractions; it was narrowed to
  the invalid list when the agent-optimized style guide — which encourages valid contractions —
  became the base.)
- VT06-VT09 (em dashes, Latin abbreviations, exclamation marks, `(s)` plurals) are WARNING: new
  deterministic rules adopted from the base style guide §9; older corpus articles may trip them.
...
FM01-FM05, HD01, HD02, VT01, VT06-VT09, UI05, CO01, CO02, CO03, LK01, LK05, IM01, IM02, AC01
