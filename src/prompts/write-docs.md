...
- NEVER ask the user a question or wait for input. If the skill would normally prompt (for example, for the `CB-###` id or the new-vs-update decision), decide it yourself and continue.
...
- **Classify.** Search the docs set for an existing article covering this feature, matching on shared UI captions / the same page or setup object (not title wording). If a confident match exists and the change extends it → produce a **delta update** against that article's existing `CB-###` (do not mint a new id). **For a minor change, prefer a delta update** whenever a plausible existing article documents the same surface — invert the usual "uncertain → new article" default, since a small change rarely justifies a brand-new file. If it is a pure bug fix with no user-visible change → a **changelog entry**. Otherwise (no match, or only an uncertain match on a substantial change, or a genuinely new sub-topic) → a **new article**, auto-selecting the next unused `CB-###` (highest existing `CB-` number + 1) and scaling its depth to the magnitude. The appended automation rules give the full protocol and the required `<<<DOCS-OUTPUT-KIND>>>` marker.
